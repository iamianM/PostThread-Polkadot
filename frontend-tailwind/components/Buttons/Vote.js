...
export default function Vote({ type, post }) {
    ...
    const fillUp = upVoteClicked ? "fill-green-500" : "fill-none"
    const fillDown = downVoteClicked ? "fill-red-500" : "fill-none"

    const { addToast } = useToasts()

    async function submitVote(value) {
        value > 0 ? setUpVoteClicked(true) : setDownVoteClicked(true)
        addToast(`Vote submitted, will finalize on the blockchain soon`, { appearance: 'info', autoDismiss: true })
        ...
        addToast(`Vote was created and posted to the blockchain.`, { appearance: 'success', autoDismiss: true })
        console.log(data)
    }
        ...
        <>
            {type === "up" ?
                <button onClick={() => submitVote(1)}>
                    <svg className={`${fillUp} hover:fill-green-500 stroke-base-content stroke-1 hover:stroke-2`}
                ...
                :
                <button onClick={() => submitVote(-1)}>
                    <svg className={`${fillDown} hover:fill-red-500 stroke-base-content stroke-1 hover:stroke-2`}
